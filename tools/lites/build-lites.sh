...
#
# The tree already has the right header. <machine/stdarg.h> resolves to
# obj/include/machine/stdarg.h, which IS this tree's
# include/i386/stdarg.h, the one patched here to use the compiler's
# varargs builtins instead of taking the address of the last named
# argument. Every varargs user in LITES -- subr_prf.c, vm_glue.c,
# e_bsd.c -- includes <machine/stdarg.h> and gets it.
#
# So plain <stdarg.h> is forwarded to that, and nothing else is
# introduced.
#
# An earlier version of this forwarded to OSFMK's sa_mach/stdarg.h
# instead, which was wrong in a way worth recording: sa_mach's
# machine/va_list.h is `typedef char *va_list`, the pre-builtin form
# this tree deliberately patched away from, so it reintroduced the old
# definition alongside the new one. It also needed two further
# forwarding headers to satisfy sa_mach's own includes. One line to the
# tree's own header replaces all three.
mkdir -p "$MR/freestanding"
cat > "$MR/freestanding/stdarg.h" <<'STDARG'
/* Generated by build-lites.sh: plain <stdarg.h>, as included by
   OSFMK's mach.h, forwarded to this tree's own varargs header so that
   no compiler-supplied header is used. */
