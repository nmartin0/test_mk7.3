...
# through OSFMK's own mach.h -- no LITES source includes it directly.
# It is forwarded to <machine/stdarg.h>, which the build resolves to
# obj/include/machine/stdarg.h, this tree's include/i386/stdarg.h.
#
# THAT IS A LITES HEADER SATISFYING A MACH INCLUDE, and it is worth
# being precise about. Forwarding to OSFMK's own sa_mach/stdarg.h was
# tried instead: it does not change the outcome, because sa_mach's
# stdarg.h itself includes <machine/stdarg.h>, which the Makerules
# include path resolves to obj/include before anything added here.
# Reaching OSFMK's copy would mean reordering the include path for the
# whole LITES build, which is a larger change than this.
#
# It is also, now, a distinction without a difference in content:
# include/i386/stdarg.h has been restored to its original definitions,
# which are the same char * form sa_mach uses.
#
# mach.h pulls three other LITES headers in the same way -- string.h,
# sys/cdefs.h and machine/ansi.h -- and has done since long before any
# of this. See docs/current-blocker.md.
mkdir -p "$MR/freestanding"
cat > "$MR/freestanding/stdarg.h" <<'STDARG'
/* Generated by build-lites.sh: plain <stdarg.h>, as included by
   OSFMK's mach.h, forwarded to the varargs header this build already
   uses, so that no compiler-supplied header is used. */
...
CC_FLAGS="-m32 -std=gnu89 -Ulinux -fno-builtin -fgnu89-inline -fcommon -fno-stack-protector -I$MR/freestanding -include $HERE/lites-compat.h"
