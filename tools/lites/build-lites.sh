...
# through OSFMK's own mach.h. OSFMK ships one, at
# $EXPORT/include/sa_mach/stdarg.h, so that is what is used instead.
#
# It is exposed through a directory of its own rather than by putting
# sa_mach on the include path, because sa_mach also carries i386/,
# sys/ and string.h, which would shadow LITES's versions of the same.
# One forwarding header exposes exactly one name.
mkdir -p "$MR/freestanding"
cat > "$MR/freestanding/stdarg.h" <<'STDARG'
/* Generated by build-lites.sh. Forwards to OSFMK's own stdarg.h so
   that no compiler-supplied header is used. */
#include <sa_mach/stdarg.h>
STDARG
#
# sa_mach/stdarg.h in turn includes <machine/va_list.h> and
# <machine/stdarg.h>, which are its OWN machine directory rather than
# LITES's. Forward those two names as well, for the same reason and in
# the same way.
mkdir -p "$MR/freestanding/machine"
cat > "$MR/freestanding/machine/va_list.h" <<'VALIST'
/* Generated by build-lites.sh. */
#include <sa_mach/machine/va_list.h>
VALIST
cat > "$MR/freestanding/machine/stdarg.h" <<'MSTDARG'
/* Generated by build-lites.sh. */
#include <sa_mach/machine/stdarg.h>
MSTDARG
