...
and the answer to "did you use the tree's own files, as intended" is
worth stating exactly.

## What was wrong

Plain `<stdarg.h>` -- which only OSFMK's `mach.h` includes; no LITES
source includes it -- was forwarded to
`export/include/sa_mach/stdarg.h`. That header's
`machine/va_list.h` is

```c
typedef	char *va_list;
```

which is **the pre-builtin form this tree deliberately patched away
from**. `include/i386/stdarg.h` is modified here to use
`__builtin_va_start` and friends, precisely because the original took
the address of the last named argument. Forwarding to sa_mach
reintroduced the discarded definition beside the new one, and needed
two more forwarding headers to satisfy sa_mach's own includes.

## What was already there

`<machine/stdarg.h>` resolves to `obj/include/machine/stdarg.h`, which
is byte-for-byte this tree's `include/i386/stdarg.h`. Every varargs
user in LITES -- `subr_prf.c`, `vm_glue.c`, `e_bsd.c` -- includes
`<machine/stdarg.h>` and gets it.

So the fix is one forwarding header pointing at the tree's own file,
replacing three that pointed at OSFMK's standalone set. Verified by
preprocessing `<stdarg.h>` with the real compile line: the chain is
`freestanding/stdarg.h` -> `obj/include/machine/stdarg.h`, and zero
occurrences of `char *va_list`.

## The honest inventory

**No compiler-supplied file is used.** Checked three ways: zero
`/usr/lib/gcc` in a full rebuild from an empty directory; no
`crtbegin`, `crtend`, `crti`, `crtn`, `libgcc` or `gcc2_compiled`
symbols in the server, emulator or kernel; and the kernel was already
clean before any of this.

**Compiler builtins are still used, and that is not a file.**
`__builtin_va_start`, `__builtin_va_arg` and `__builtin_va_list` are
generated by the compiler, not linked from it, and using them is this
tree's own deliberate choice in `include/i386/stdarg.h`.

**One file is new and is not from the tree**: `libkern/quaddiv.c`, the
four wrappers `__divdi3`, `__udivdi3`, `__moddi3` and `__umoddi3`.
4.4BSD has them as `sys/libkern/*di3.c`; LITES never shipped them and
neither does any copy in the reference collection. They are written
against `quad.h`'s declaration of `__qdivrem` rather than copied, and
tested against the compiler's own 64-bit division over 200,000 cases
before being trusted.

**Two files are used as intended for the first time.**
`libkern/qdivrem.c` shipped with LITES, was absent from `conf/files`
and had never been compiled -- because nothing referenced it. It is
now built and called, which is what it was there for.
`include/i386/stdarg.h` is likewise now reached by the one include
that previously escaped to gcc.

**One generated shim remains**, `$MR/freestanding/stdarg.h`, a
one-line forward. It exists because the include is spelled
`<stdarg.h>` in OSFMK's `mach.h` and the tree's header is at
`<machine/stdarg.h>`; nothing in either tree spells it the other way.
That is a build-time bridge, not a source change, and it points at the
tree's own file.
